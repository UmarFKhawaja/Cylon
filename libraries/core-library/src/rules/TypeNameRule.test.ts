...
import { NonVoidTypeName, VoidTypeName } from '../models';
import { TypeNameRule } from './TypeNameRule';

describe('TypeNameRule', (): void => {
  it('should match the \'Void\' string', (): void => {
    const input: Input = Input.fromText('Void');

    const result: Result = applyRule(input, TypeNameRule);
