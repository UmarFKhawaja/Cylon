...
import { CloseCurlyBracketRule } from './CloseCurlyBracketRule';

describe('CloseCurlyBracketRule', (): void => {
  it('should match a close curly bracket character', (): void => {
    const input: Input = Input.fromText('}');

    const result: Result = applyRule(input, CloseCurlyBracketRule);
