...
import { CloseParenthesisRule } from './CloseParenthesisRule';

describe('CloseParenthesisRule', (): void => {
  it('should match a close parenthesis character', (): void => {
    const input: Input = Input.fromText(')');

    const result: Result = applyRule(input, CloseParenthesisRule);
