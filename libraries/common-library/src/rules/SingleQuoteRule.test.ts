...
import { SingleQuoteRule } from './SingleQuoteRule';

describe('SingleQuoteRule', (): void => {
  it('should match a single quote character', (): void => {
    const input: Input = Input.fromText('\'');

    const result: Result = applyRule(input, SingleQuoteRule);
