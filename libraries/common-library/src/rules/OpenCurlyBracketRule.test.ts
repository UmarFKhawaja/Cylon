...
import { OpenCurlyBracketRule } from './OpenCurlyBracketRule';

describe('OpenCurlyBracketRule', (): void => {
  it('should match a open curly bracket character', (): void => {
    const input: Input = Input.fromText('{');

    const result: Result = applyRule(input, OpenCurlyBracketRule);
