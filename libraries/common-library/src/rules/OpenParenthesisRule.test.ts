...
import { OpenParenthesisRule } from './OpenParenthesisRule';

describe('OpenParenthesisRule', (): void => {
  it('should match a open parenthesis character', (): void => {
    const input: Input = Input.fromText('(');

    const result: Result = applyRule(input, OpenParenthesisRule);
