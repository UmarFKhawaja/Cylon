...
describe('RoutesKeywordRule', (): void => {
  it('should match the \'routes\' keyword', (): void => {
    const input: Input = Input.fromText('routes');

    const result: Result = applyRule(input, RoutesKeywordRule);
    ...
    expect(keywordToken.keyword).toEqual('routes');
  });
});
