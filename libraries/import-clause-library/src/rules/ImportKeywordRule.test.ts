...
describe('ImportKeywordRule', (): void => {
  it('should match the \'import\' keyword', (): void => {
    const input: Input = Input.fromText('import');

    const result: Result = applyRule(input, ImportKeywordRule);
    ...
    expect(keywordToken.keyword).toEqual('import');
  });
});
