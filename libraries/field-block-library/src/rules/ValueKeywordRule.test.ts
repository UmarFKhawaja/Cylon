...
describe('ValueKeywordRule', (): void => {
  it('should match the \'value\' keyword', (): void => {
    const input: Input = Input.fromText('value');

    const result: Result = applyRule(input, ValueKeywordRule);
    ...
    expect(keywordToken.keyword).toEqual('value');
  });
});
